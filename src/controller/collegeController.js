...
const createColleges = async function (req, res) {
  try {
    ...
    if (!validator.isValid(fullName)||/\d/.test(name)) {
      res.status(400).send({ status: false, msg: "College Fullname is required OR name cannot have numbers" });
        return;
    }
    ...
    res
      .status(201).send({status: true, msg: "New College created successfully", data: newCollege,});
  } catch (error) {
