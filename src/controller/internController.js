...
const createInterns = async function (req, res) {
    try {
        ...
        const { name, mobile, email, collegeId} = requestBody;
        ...
        let FindId = await authorModel.findById(CollegeId)
        if (!FindId) return res.status(400).send({ status: false, msg: 'CollegeId does not exist'})
       
        // if (!validator.isValid(isDeleted)) {
        //     res.status(400).send({ status: false, msg: " is required" });
        //     return;
        // }
